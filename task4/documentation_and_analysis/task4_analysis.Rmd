...
Organising the treat rating columns, I discover that there are numerous columns that are meant to match up, but have different names, so I return and amend those before the bind happens.

  select(1:york_peppermint_patties) %>% 
  select(1:6, sort(peek_vars()))
